function init_sidebar() {
	...
	$SIDEBAR_MENU.find("a").on("click", function (b) {
		var c = $(this).parent();
		c.is(".active") ? (c.removeClass("active active-sm"), $("ul:first", c).slideUp(function () { a() }))
		...
		$MENU_TOGGLE.on("click", function () {
			$BODY.hasClass("nav-md") ? ($SIDEBAR_MENU.find("li.active ul").hide(),
				$SIDEBAR_MENU.find("li.active")
					.addClass("active-sm")
					.removeClass("active")) : ($SIDEBAR_MENU.find("li.active-sm ul")
						.show(), $SIDEBAR_MENU.find("li.active-sm")
							.addClass("active")
		...
		$SIDEBAR_MENU.find("a")
			...
			.parents("ul").slideDown(function () { a() })
			.parent().addClass("active"),
		$(window).smartresize(function () {
			a()
		}),
		a(),
		$.fn.mCustomScrollbar && $(".menu_fixed")
			.mCustomScrollbar({
				autoHideScrollbar: !0, theme: "minimal",
				mouseWheel: { preventDefault: !0 }
			})
} function countChecked() {
	"all" === checkState && $(".bulk_action input[name='table_records']")
		.iCheck("check"), "none" === checkState && $(".bulk_action input[name='table_records']")
			.iCheck("uncheck");
	var a = $(".bulk_action input[name='table_records']:checked").length;
	a ? ($(".column-title").hide(), $(".bulk-actions")
		.show(), $(".action-cnt").html(a + " Records Selected")) :
				...
				series:
					{
						...
						points: { radius: 0, show: !0 }, shadowSize: 2
					}, grid: { verticalLines: !0, hoverable: !0, clickable: !0, tickColor: "#d5d5d5", borderWidth: 1, color: "#fff" }, colors: ["rgba(38, 185, 154, 0.38)", "rgba(3, 88, 106, 0.38)"], xaxis: { tickColor: "rgba(51, 51, 51, 0.06)", mode: "time", tickSize: [1, "day"], axisLabel: "Date", axisLabelUseCanvas: !0, axisLabelFontSizePixels: 12, axisLabelFontFamily: "Verdana, Arial", axisLabelPadding: 10 }, yaxis: { ticks: 8, tickColor: "rgba(51, 51, 51, 0.06)" }, tooltip: !1
                ...
                $("#fc_create").click(),
                    ...
                    $(".antosubmit").on("click", function () {
                        ...
                        return b && (ended = b),
                            ...
                            $(".antoclose").click(), !1,
                            document.getElementById("gender1R").checked = true;
                        document.getElementById("gender2R").checked = false;
                    })
            },
            eventClick: function (a, b, c) {
                $("#fc_edit").click(),
                    $("#custnameSched").val(a.title),
                    $("#custaddressSched").val(a.address),
                    $("#custcontnumSched").val(a.contnum),
                    $("#custservtype").val(a.address == "" ? 1 : 2),
                    $("#custservSched").val(a.services),
                    $("#custgenderSched").val(a.sex),
					$('#custtotamtSched').val(a.amount),
					f = $("#event_type").val(),
					selectedService = a._id,
					serviceSelc = a.sID,
                    $(".antosubmit2").off().on("click", function () {
                        a.title = $("#title2").val(),
                            a.addressR = $("#addressR2").val(),
                            a.contnumR = $("#contnumR2").val(),
                            a.selectedServR = $("#selectedServR2").val(),
                            a.totAmtNsR = $("#totAmtNsR2").val(),
...
$('#fc_edit').click(function () {
	document.getElementById("btnConfReserv").disabled = false;
	document.getElementById("btnCancReserv").disabled = false;
	$('.sched-details-none').hide();
	$('.sched-details').show();
	$('#custnameSched').val();
	$('#custgenderSched').val("Male");
	$('#custtherapSched').val("Dobby");
	$('#custaddressSched').val("");
	$('#custcontnumSched').val("");
	$('#custservSched').val("Swedish Massage");
	$('#custtotamtSched').val("399.00");
});
	...
	if (console.log("run_datatables"),
		...
		var a = function () {
			$("#datatable-buttons").length && $("#datatable-buttons").DataTable
				({
					dom: "Blfrtip", buttons: [{ extend: "copy", className: "btn-sm" },
